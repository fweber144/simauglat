...
# remotes::install_github("stan-dev/projpred")
###
...
if (only_init_fit) {
  ...
  cat("-----\n")
  stopifnot(all.equal(par_ratio_sigti, 0.259129881, tolerance = 1e-8))
  ###
  bfit <- brms::brm(
...
gg_perf_indiv_abs <- function(da_info, width = 5.2, height = width * 0.618) {
  da_prep <- da_info$da_prep
  indiv_txt <- da_info$indiv_txt
  eval_scale <- da_info$eval_scale
  perf_scale <- da_info$perf_scale
